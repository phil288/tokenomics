...
- `paceMarker(p, invert)` is the tick on the track; `paceNote(p)` is the line under it (`day 3/7 · 14.3%/day · budget 43% · 26% still available`, red `.over` when `spare < 0`). **`invert`** mirrors the tick to `100 − budget` for Antigravity, whose bars fill with **remaining** quota, not used.
