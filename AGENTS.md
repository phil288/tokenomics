...
- **`src/web/*.js`** — the client-side dashboard, split into ES modules (served by `server.js` under `/web/`):
  ...
  - `layout.js` — free-drag card layout. Saved layouts may be partial because disabled providers have no captured position. When such providers become visible, `placeUnmappedVisible()` uses the pure `layout-placement.js` collision helper to assign and persist non-overlapping positions before `.arranged` absolute positioning is applied; never leave a visible arranged card without a layout entry. Placement is constrained by the viewport’s usable width: saved cards keep their saved row and are only clamped horizontally, while newly visible cards are placed underneath when no space is available. Overview grid tracks use `minmax(0, 1fr)` and zero minimum widths to prevent content-driven horizontal overflow.
    - **`placeUnmappedVisible()` runs on every SSE render tick** (via `reapplyCardLayout()`, called from `main.js`'s 10 s `render()` and on every `viewchange`/visibility toggle) — not just when a provider is enabled. It measures the board's live `clientWidth`, and if a saved card's `x + w` no longer fits that width it "clamps" the saved position and marks the layout `changed`, which auto-`persistLayout()`s (POSTs to `/api/settings`, no user action). A board mid-reflow — a provider's `display` just toggled, the 1100 px breakpoint just crossed, first paint before CSS/fonts settle — can transiently measure narrower than its real width, which clamps perfectly valid saved positions and silently persists the corruption. This was the cause of layouts "randomly resetting": no drag required, just a badly-timed render tick. Fix: `placeUnmappedVisible()` bails (`return false`, no clamp, no persist) whenever `boardWidth < MIN_W` — a board narrower than one card's minimum width is not a real measurement worth trusting. Any future change to the clamp logic must keep this sanity check (or an equivalent one) ahead of it. Contract test: `test/board-layout.test.js` (`placeUnmappedVisible bails on an implausible board-width measurement`).
