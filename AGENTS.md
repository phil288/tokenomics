...
- **Dedupe is per `(key, level, unit index)`** — at most one warn + one over per bar per day (per hour for the 5 h session), re-armed when the window's unit advances (a new day raises the budget, so the alert means something again). An `over` alert also consumes that unit's `warn` slot so a jump past the budget doesn't ping twice.
...
- Tests: `test/notify.test.js` (thresholds, dedupe/re-arm, permission degradation, wiring contract — loads the module with a stubbed `globalThis.Notification`), `test/settings.test.js` (clamping/persistence), `test/settings-tabs.test.js` (the `alerts` tab/panel pair — its `TABS` list must include every tab).
