...
- **Collector modules**: `src/collectors.js` is the small public facade/orchestrator. Tool-specific logic lives in `collectors-rtk.js`, `collectors-caveman.js`, `collectors-headroom.js`, `collectors-cursor.js`, `collectors-antigravity.js`, `collectors-claude.js`, and `collectors-activity.js`; shared home/path/I/O helpers live in `collector-utils.js`. Keep `collectors.js` under 500 lines and add new collector behavior to the relevant tool module rather than growing the facade.
- **`index.html`**:
...
- **`src/web/*.js`** — the client-side dashboard, split into ES modules (served by `server.js` under `/web/`):
  ...
  - `charts.js` — the RTK daily bar chart and history trend lines (**Chart.js**, loaded from a CDN). Live SSE history rows must recompute Headroom's `rawUsd`/`usd`/`saved` window-cost fields from `window_tokens.by_model` using the shared client pricing helpers; do not carry those fields forward from the previous row. The saved-provider trend can be empty when RTK, Caveman, and Headroom are all hidden, so `renderHistory()` must clear/destroy the existing saved chart in that state. The range selector is a single labeled ARIA radio group (`role="radiogroup"` with `role="radio"`/`aria-checked`).
  - `pricing.js` — the client `PRICING` matrix, `derivePricingRates(prefix, input)` (settings-table auto-fill), and per-model cost/weight math. Import-free so the CJS test suite can load it (see `test/pricing.test.js`).
  ...
  - `layout.js` — free-drag card layout. Saved layouts may be partial because disabled providers have no captured position. When such providers become visible, `placeUnmappedVisible()` uses the pure `layout-placement.js` collision helper to assign and persist non-overlapping positions before `.arranged` absolute positioning is applied; never leave a visible arranged card without a layout entry. Overview grid tracks use `minmax(0, 1fr)` and zero minimum widths to prevent content-driven horizontal overflow.
    - **Writes to the layout map are user-initiated only.** `reapplyCardLayout()` runs on every SSE render tick (10 s), every `viewchange`, and every visibility toggle, so anything it persists is persisted *without a user action*. The rule is therefore: the saved map (`state.cardLayout` / `state.analysisLayout`) is written **only** by a drag/resize (`finish()`), by `seedBoard()` on entering arrange mode, and by `placeUnmappedVisible()` assigning a slot to a widget that has **no** saved position. Everything else is paint.
      - **Clamping is render-only.** `applyBoard()` clamps a saved `x`/`w` that no longer fits the measured board width when *painting*, and deliberately never writes the clamped value back — so re-widening the window restores the user's original placement. Earlier builds clamped inside `placeUnmappedVisible()` and marked the layout `changed`, which auto-`persistLayout()`d. Any transient narrow measurement — a vertical scrollbar appearing as card content grows, the 1100 px breakpoint being crossed, a provider's `display` toggling, first paint before CSS/fonts settle — then rewrote perfectly valid saved positions and POSTed the corruption. That is the real cause of layouts "randomly resetting": no drag required, just a badly-timed render tick.
      - `placeUnmappedVisible()` still bails (`return false`, no placement, no persist) whenever `boardWidth < MIN_W` — a board narrower than one card's minimum width is not a measurement worth trusting. Keep this check (or an equivalent) ahead of any placement logic.
      - **Below 1101 px the free layout is not applied at all.** The CSS grid drops to 2/1 columns there, where a layout captured on a wide screen is meaningless. `applyLayout()`/`reapplyCardLayout()` call `unapplyBoard(b)` (removes `.arranged` + inline `left/top/width/height`, clears `minHeight`) instead — the saved map is untouched on disk, so widening the window brings the layout back. Arrange mode overrides the gate. `unapplyBoard()` must never touch a layout map.
      - A debounced `window.addEventListener('resize')` re-runs `reapplyCardLayout()` (150 ms) so both shrinking and re-widening repaint.
      - Contract tests: `test/board-layout.test.js` (`placeUnmappedVisible bails on an implausible board-width measurement`), `test/layout-placement.test.js` (paint-only clamp + single map write, narrow-viewport fallback, resize repaint).
    - `src/settings.js`'s `cleanLayout()` (drops `__proto__`/`constructor` keys and non-finite `x/y/w/h`) is wired into `updateSettings()` for **both** `CARD_LAYOUT` and `ANALYSIS_LAYOUT` — it used to be dead code (written, never called), so a malformed client payload (e.g. `NaN` from `parseFloat` on an unset `el.style.left`) was written to `data/settings.json` unsanitized. Test: `test/settings.test.js` (`updateSettings sanitizes CARD_LAYOUT`).
    - Boot ordering in `initSettingsAndPricing()` (`src/web/settings.js`): `setCardLayout`/`setAnalysisLayout` must run **before** `applySavedProviderVisibility()`, because the latter triggers `reapplyCardLayout()` — running it while `state.cardLayout` is still `{}` risks seeding/persisting an empty layout if `reapplyCardLayout()`'s empty-map short-circuit is ever changed.
...
- **`data/`** (gitignored):
  ...
  - `src/history.js` exposes a stable in-memory `history` array that `server.js` captures for `/api/history`. Loading and `HISTORY_MAX` trimming must mutate that array in place (`splice`/`length`), never reassign it; replacing the array freezes the endpoint on its old 5,001-row reference while the recorder silently continues on a new array. Large loads must repopulate it iteratively rather than spreading all rows into one `splice()` call, because `HISTORY_MAX` is environment-configurable and can exceed the JavaScript argument limit.
...
- **Derived rates from Input.** Cache columns are a function of the input price: read `0.1×`, write-5m `1.25×`, write-1h `2×`. Output is family-specific: Claude/Cursor `5×`, Gemini/Antigravity `6×` (prefix `startsWith`). `derivePricingRates(prefix, input)` in `pricing.js` is the single formula; the settings Pricing tab applies it live on `.px-in` / `.px-prefix` `input` events (`applyDerivedRates` in `settings.js`) **and** via `#btn-refresh-pricing` (`refreshPricingRates()` walks every row). Do **not** recompute on modal open — a saved row may carry a custom override; Refresh rates is the explicit snap-back. Tests: `test/pricing.test.js` (math + both default tables match the formula), `test/settings-tabs.test.js` (live wiring + refresh button).
- **Prefix collisions: longer/more-specific prefix must sort first.** `priceFor()`/`derivePricingRates` match by `name.startsWith(prefix)` and return the first hit, so a `-fast` variant row (e.g. `cursor-grok-4.6-fast`) must be listed **before** its shorter base prefix (`cursor-grok-4.6`) — otherwise the base row shadows it and the fast variant never gets its own (higher) price.
- **Not every real-world row fits the derived formula, and that's fine.** Third-party models Cursor bills at the vendor's own published rate (e.g. xAI's Grok 4.6/4.5 and their `-fast` variants, Composer 2.5/2.5-fast) don't follow the generic 5×/6× output or 0.1/1.25/2× cache ratios — hand-enter their real quoted numbers instead of forcing the formula. Likewise a model's *introductory* pricing (e.g. Gemini 3.7/3.8 Flash launched at 5× output instead of Gemini's usual 6×) can diverge from the family formula until it reverts to standard pricing. `test/pricing.test.js` has a `FORMULA_EXEMPT_PREFIXES` allowlist for these — add a prefix there (with a comment citing the real source numbers) rather than distorting the price to satisfy the formula check.
- **`data/settings.json` (gitignored) can shadow code defaults.** Once a user has ever opened the Pricing tab (or the app has written settings at all), `data/settings.json` holds its own persisted `PRICING` array that `loadSettings()` merges *over* `DEFAULT_PRICING` — so editing `src/settings.js` alone does not change what a running instance shows. Update `data/settings.json`'s `PRICING` to match too (or have the user click "Restore" / delete the key) when shipping a pricing-table change to an already-configured install.
...
- `process.env.RTK_DATA_HOME` takes precedence over the persisted Configuration Settings value. This is required for service launchers and isolated tests to force a data home even when `data/settings.json` already contains a machine-specific path.
...
- **Quota freshness.** Headroom no longer supplies the Claude card's quota bars (see §4.6) — `latest.five_hour` / `seven_day` in `subscription_state.json` are still read for its own telemetry, but the card ignores them. The freshness line now reports the `claude /usage` poll (§4.6).

### 4. Cursor
- Queries the Connect RPC endpoint `https://api2.cursor.sh/aiserver.v1.DashboardService/GetCurrentPeriodUsage` to fetch account quotas and billing cycles. The card's white pace ticks (`.pace-marker`) need that cycle: `cursorCycle()` reads `billingCycleStart` / `billingCycleEnd` (ms strings on the live payload) via `cycleFromRange`. The old aliases `subscriptionCycleStart` / `billingCycle.cycleStart` are fallbacks only — they are **not** what GetCurrentPeriodUsage sends today, and reading only them left every Cursor bar without a tick or `.pace-note`.
...
### 6. Claude quota (`claude /usage`)

- The Claude card's quota bars come from **Claude Code's own `/usage` slash command**, not from Headroom's mirror of the Anthropic quota API. This removes the dependency on a third-party proxy still running and still polling.
- **No PTY needed** (unlike `agy`, §4.5): `claude -p "/usage"` renders fine to a pipe. Do **not** add a Python driver for it.
- **Cost ~11 s per call** — longer than `REFRESH_MS` (10 s). So it runs on its own slow timer, `pollClaude()` on `CLAUDE_POLL_MS` (default `300000` = 5 min), and `collectStats()` only reads the cache. Same pattern as `pollAntigravity()`. **Never** put it in the fast SSE loop: every frame would block on an 11 s subprocess.
- `src/collectors-claude.js` owns it. `parseClaudeUsage()` maps the CLI's labels onto the keys the card already renders, so per-model window discovery, pacing, alerts and history snapshots work unchanged:
  - `Current session:` → `five_hour`
  - `Current week (all models):` → `seven_day`
  - `Current week (Fable):` → `seven_day_fable` (any model → `seven_day_<slug>`)
- **Reset-timestamp traps.** The CLI prints local wall-clock, zone-named, **with no year**: `resets Aug 26, 10:59pm (Europe/Paris)`. `parseResetAt()` handles both traps — it rolls forward a year when the naive result lands >30 days in the past (a December→January reset would otherwise resolve into the past), and resolves the zone offset **twice** because the offset itself shifts across a DST boundary. Uses `Intl.DateTimeFormat` only — **no tz dependency** (zero-dep rule). Also note the printed minute drifts between calls (the CLI rounds a relative time), which is why the reset is stored as an absolute ISO instant rather than trusted as a string.
- **The contributor block is deliberately discarded.** `/usage` also prints "What's contributing to your limits usage?" (24h/7d request + session counts, parallel-session %, subagent-heavy %, top subagents). The card renders quota bars only; a test asserts none of that data leaks into the quota object. Surfacing it later means new markup + CSS + card tests.
- **A failed poll must not blank the card.** The poller keeps the last good `latest` and sets `stale: true` + `error`. So `renderClaude`'s guard is `if (!d)` — **not** `if (!d || d.error)`, which would throw away valid bars on one bad poll. The freshness line reports how old they are.
- Data reaches the card as **`stats.claude`** (`buildClaude()` in `collectors.js`), which carries the poll cache plus `users` and Headroom's `health` (the card still shows the health pill). `polled_at` sits at the **top level**, not under `latest`. Per-user `claude.latest` was dropped: quota is account-wide, not per-home.
...
- `test/settings-tabs.test.js` — the "Restore absolute totals" control exists in the Data tab and is wired to `DELETE /api/baseline`.
  - `test/provider-visibility.test.js` — DOM/JS contract for immediate provider visibility, hero/trend filtering, and the dashboard-level Trends/cost exception.
  - `test/layout-placement.test.js` — pure collision placement plus integration guards that reserve saved positions belonging to hidden cards.
  - `test/chart-fallback.test.js` — executable no-`Chart` Canvas fallback coverage, including cost history remaining visible when the Headroom card is disabled and fresh SSE snapshots extending stale persisted chart history without duplicate timestamps.
  - `test/trends-range.test.js` — executable range normalization/cutoff coverage plus the linear timestamp-axis and accessible pressed-state contracts.
...
- Window lengths per source: Claude — constants at the call site (`5 * HOUR`, `7 * DAY`, incl. every discovered `seven_day_<model>`); Antigravity — `windowSecsFromLabel(lim.label)` ("Weekly" → 7 d) with `remainingSecs` from `parseDuration(lim.refresh)` (agy only gives the rendered `"101h 16m"` string; a `full` limit has no refresh string → no pace); Cursor — `cycleFromRange(billingCycleStart, billingCycleEnd)` uses the current period's start→renewal window from `GetCurrentPeriodUsage` (ms / ms-string / unix seconds / ISO via `parseEpochMs`). Older payloads that only have `subscriptionCycleStart` / `billingCycle.cycleStart` still work: missing end falls back to `monthlyCycle(start)`, which walks the billing anniversary forward past now (28–31 d) instead of assuming 30. **Gotcha:** the live field names are `billingCycleStart` / `billingCycleEnd`, not `subscriptionCycleStart` — reading only the old aliases made `computePace` return null, so the white `.pace-marker` ticks and `.pace-note` lines never rendered.
